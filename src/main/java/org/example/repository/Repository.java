...
While the use of interfaces are important to OCP and DIP, being able to have a repository utilising any interface means its open to extension and reuse in other environments.
...
public class Repository<T> {
    ...
    RepositoryPrinter<T> printer = new RepositoryPrinter<>();
    ...
    And when addressed further on the issues that can occur it recommended me to;
    ...
    *Backwards compatibility issues as toList() is not available in earlier versions (Used to be Collectors::toList())
    *Concurrency issues,  Not relevant due to the lack of multiple threads

    At one point it suggested a type generic implementation with the reasoning that the type generic implementation means a developer cannot enter a type value that doesn't match the filter policy.
    But may use several types of filter policies targeting different types.
    ...
    Furthermore, By keeping with the functional interfaces of Predicates we can ensure that anyone use existent functionality when filtering.
    ...
    it came back to this solution once more but both suggested and advised against extracting helper methods. Specifically the predicate.test()
